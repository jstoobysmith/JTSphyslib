...
This file builds the Stinespring (unitary) dilation of a completely positive map given
by a Kraus family `K : r → Matrix m m R`.

Rather than introducing its own notions of completely positive maps and partial traces, this
file reuses the definitions from `QuantumInfo`:

* a Kraus map `ρ ↦ ∑ i, K i * ρ * (K i)ᴴ` is `MatrixMap.of_kraus K K`
  (see `QuantumInfo.Channels.MatrixMap`);
* the partial trace over the second factor `Tr₂` is `Matrix.traceRight`, and the partial trace
  over the first factor is `Matrix.traceLeft` (see `QuantumInfo.ForMathlib.Matrix`).

What remains here are the genuinely new, Stinespring-specific constructions (the dilation operator
`stinespringOp`, the unitary dilation `Ud`, the Kraus completion `krausCompletion`, …) together
with the theorems relating them to the `QuantumInfo` definitions.
-/
...
/-- Entrywise application of `MatrixMap.of_kraus`: `(of_kraus M N) ρ = ∑ k, M k * ρ * (N k)ᴴ`.
This is the bridge between the `QuantumInfo` Kraus map and the explicit Kraus sum used below. -/
theorem MatrixMap.of_kraus_apply {A B κ R : Type*} [Fintype A] [DecidableEq A] [Fintype κ]
    [CommSemiring R] [Star R] (M N : κ → Matrix B A R) (ρ : Matrix A A R) :
    MatrixMap.of_kraus M N ρ = ∑ k, M k * ρ * (N k)ᴴ := by
  simp only [MatrixMap.of_kraus, LinearMap.coe_sum, LinearMap.coe_mk, AddHom.coe_mk,
    Finset.sum_apply]

/-- Quantum channel: the Kraus operators of a completely positive, trace-preserving map. -/
def QuantumChannel {R : Type*} [Mul R] [One R] [Star R] [AddCommMonoid R]
    {q r : Type*} [Fintype q] [Fintype r] [DecidableEq q]
    (K : r → Matrix q q R) :=
  ∑ i, (K i)ᴴ * K i = 1

/-- Quantum operation: the Kraus operators of a completely positive, trace non-increasing map. -/
...
/-- The partial trace of the Stinespring dilation, using `Matrix.traceRight`. -/
def stinespringForm {R : Type*} [Ring R] [StarRing R]
    {m r : Type*} [Fintype r] [DecidableEq r] [Fintype m]
    (K : r → Matrix m m R) :=
  fun ρ => (stinespringDilation K ρ).traceRight
...
lemma tr₂_e₀Xe₀ {R : Type*} [RCLike R]
    ...
    (ρ : Matrix m m R) :
    (ρ ⊗ₖ e).traceRight = ρ := by
  ext i j
  simp only [Matrix.traceRight, kroneckerMap, of_apply]
  have :  ∑ x, ρ i j * e x x
...
def stinespringUnitaryForm {R : Type*} [RCLike R] {m r : ℕ}
    ...
    let U := Ud hK z
    (U * (ρ ⊗ₖ (single z z 1)) * Uᴴ).traceRight
...
theorem tracefree_version {R : Type*} [RCLike R]
  ...
  simp only [Fin.isValue, Matrix.mul_apply, conjTranspose_apply, star_def, kroneckerMap_apply,
    Matrix.one_apply, mul_ite, mul_one, mul_zero, Matrix.traceRight, of_apply]
  ring_nf;
...
theorem heisenberg_schrõdinger {R : Type*} [RCLike R]
  ...
  let schrõdinger := (V * ρ * Vᴴ).traceRight; -- evolve the state forward: V = V(t), ρ = ρ(0)
...
def generalForm {R : Type*} [RCLike R]
    ...
    fun ρ => (U * (ρ ⊗ₖ (single z z 1)) * Uᴴ).traceRight
...
theorem unitaryForm_of_general {R : Type*} [RCLike R] {m r : ℕ}
  ...
  unfold
    stinespringUnitaryForm Matrix.traceRight Ud
    stinespringGeneralForm dilation general_dilation
  ext a b
  simp only [of_apply]
  congr
  ext c
...
theorem unitaryForm_of_general_e {R : Type*} [RCLike R] {m r : ℕ}
  ...
  unfold
    stinespringUnitaryForm_e Matrix.traceRight Ud
    stinespringGeneralForm_e dilation general_dilation
  ext a b
  simp only [of_apply]
  congr
  ext c
...
lemma stinespringGeneralForm_works {R : Type*} [RCLike R] {m r : ℕ}
    ...
    stinespringGeneralForm K z M = ⇑(MatrixMap.of_kraus K K) := by
      -- my 4.27 proof failed in 4.31 so this is Aristotle:
      ext ρ
      rw [MatrixMap.of_kraus_apply]
      unfold stinespringGeneralForm dilation general_dilation stinespringOp Matrix.traceRight;
      ext i j;
      simp only [Fin.isValue, Matrix.sum_apply, kroneckerMap_apply, Matrix.mul_apply, ite_mul,
        conjTranspose_apply, star_def, of_apply];
...
lemma stinespringUnitaryForm_works {R : Type*} [RCLike R] {m r : ℕ}
    {K : Fin r → Matrix (Fin m) (Fin m) R}
    (hK : ∑ i, (K i)ᴴ * K i = 1) (z : Fin r) :
    stinespringUnitaryForm hK z = ⇑(MatrixMap.of_kraus K K) := by
...
lemma partialTrace_tensor {R : Type*} [RCLike R] {m n : ℕ}
    ...
    (A ⊗ₖ B).traceRight = (trace B) • A  := by
  ext i j
  simp only [Matrix.traceRight, kroneckerMap, trace, of_apply, diag_apply, smul_apply,
    smul_eq_mul]
...
lemma trace_tr₂ {R : Type*} [RCLike R] {m n : ℕ}
    ...
    trace ρ = trace ρ.traceRight := (Matrix.traceRight_trace ρ).symm
...
theorem stinespringForm_eq {R : Type*} [RCLike R] {m r : ℕ}
    (K : Fin r → Matrix (Fin m) (Fin m) R)
    (ρ : Matrix (Fin m) (Fin m) R) :
    (stinespringDilation K ρ).traceRight = MatrixMap.of_kraus K K ρ := by
  rw [MatrixMap.of_kraus_apply]
  unfold Matrix.traceRight stinespringDilation
  ext i j
