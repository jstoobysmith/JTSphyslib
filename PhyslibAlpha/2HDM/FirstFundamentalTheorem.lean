...
/-!
# The first fundamental theorem for the two Higgs doublet model

## i. Overview

The **First Fundamental Theorem** states that a gauge-invariant polynomial effective potential is a
polynomial in the four Gram bilinears `gramVector`. The proof reduces the potential to the
polynomial family of orbit representatives `repHiggs X` and runs the following pipeline:

1. **Charge balancing.** Gauge (torus) invariance forces the slice potential, written in
   hypercharge eigen-coordinates, to be supported only on hypercharge-neutral monomials.
2. **Generation.** Every neutral monomial is a product of the five neutral quadratic bilinears,
   so the slice potential is a polynomial in them (Condition A).
3. **Denominator clearing.** A power of `‖Φ1‖²` (resp. `‖Φ2‖²`, via the doublet swap) times the
   potential is a genuine Gram polynomial.
4. **Coprimality.** `‖Φ1‖²` and `‖Φ2‖²` are coprime in the (algebraically independent) Gram ring,
   which removes the denominators and yields the Gram polynomial.

## ii. Key results

* `exists_polynomial_repHiggs_realGen` — Condition A: on representatives, the value is a polynomial
  in the five real bilinear generators.
* `exists_clearing_all`, `exists_clearing_all_snd` — clearing the `‖Φ1‖²` and `‖Φ2‖²` denominators.
* `exists_polynomial_on_repHiggs` — the invariant-theoretic core on the representative family.
* `effectivePotential_is_polynomial_gramVector` — the First Fundamental Theorem.

## iii. Table of contents

* A. Gauge invariance as invariance of the slice polynomial under the torus
* B. Hypercharge eigen-coordinates and charge balancing
* C. Generation of neutral monomials by the bilinears
* D. Condition A: the value is a polynomial in the bilinear generators
* E. Clearing the `‖Φ1‖²` and `‖Φ2‖²` denominators
* F. Algebraic independence and coprimality of the Gram generators
* G. The first fundamental theorem

## iv. References

The statement is the `SU(2)` first fundamental theorem specialised to two doublets in `ℂ²`.
...
Gauge (torus) invariance of the potential forces the slice polynomial `P` to be invariant under the
hypercharge rotations of its variables: the Cartan rotation `rotSubst` and the residual `U(1)`
rotation `resSubst`.
-/
...
Changing to hypercharge eigen-coordinates `z, z̄, w₀, w̄₀, w₁, w̄₁` diagonalises the torus rotation
into a scaling by the hypercharges `chargeA` (Cartan) and `chargeB` (residual). Feeding an
infinite-order phase into the invariance from part A shows that every monomial of the slice
...
complex value back to its real part, turns the generation result of part C into **Condition A**: the
value `V (repHiggs X)` is a polynomial in the five real bilinear generators
`‖Φ1‖², Re⟪⟫, Im⟪⟫, |Φ2₀|², |Φ2₁|²`.
-/
...
Condition A expresses the value as a polynomial in the bilinear generators, but two of them —
`‖Φ2₀‖²` and `‖Φ2₁‖²` — are not directly Gram polynomials. Multiplying by a power of `‖Φ1‖²`
clears these denominators; the doublet swap of part `SwapDoublet` then gives the mirror statement
with `‖Φ2‖²`.
-/
...
The four Gram generators are algebraically independent (`gramP_injective`), and the two linear
combinations `‖Φ1‖² = (g₀+g₃)/2` and `‖Φ2‖² = (g₀-g₃)/2` are coprime in the Gram ring
(`uPow_dvd`). Together these let the `‖Φ1‖²` and `‖Φ2‖²` powers be cancelled.
-/
...
The two structural ingredients from elsewhere are that every configuration is gauge equivalent to a
representative `repHiggs X` (`exists_smul_eq_repHiggs`) with a polynomial Gram vector
(`gramVector_repHiggs_*`). Combining the two denominator clearings of part E with the coprimality of
part F removes the `‖Φ1‖²`/`‖Φ2‖²` powers and produces the Gram polynomial on representatives
(`exists_polynomial_on_repHiggs`); gauge invariance then transports it to all configurations.
-/

open MvPolynomial in
/-- **First fundamental theorem (representative form).** A gauge-invariant polynomial potential,
  restricted to the polynomial family of orbit representatives `repHiggs X`, is a polynomial in the
  Gram components of that family. This is the non-abelian `SU(2)` first fundamental theorem
  specialised to two doublets in `ℂ²`, proved by the doublet-swap argument: clearing the `‖Φ1‖²`
  denominator (aligning `Φ1`) and the `‖Φ2‖²` denominator (aligning `Φ2`, via the gauge-commuting
  swap), then using that `‖Φ1‖²` and `‖Φ2‖²` are coprime in the Gram ring. -/
...
/-- **The first fundamental theorem.** An invariant effective potential with maximum mass dimension
  `n` can be written as a polynomial in the entries of the Gram vector. -/
