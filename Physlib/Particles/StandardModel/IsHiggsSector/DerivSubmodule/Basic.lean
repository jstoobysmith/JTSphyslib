...
/-- The submodule of `B` generated by the Higgs symbols and their conjugates carrying
  `n` derivatives. -/
noncomputable def derivSubmodule (h : IsHiggsSector B rep repLorentz massWeightPoly)
    (n : ℕ) : Submodule ℂ B :=
...
lemma derivSubmodule_le_ker_H {n k : ℕ} (d : Fin k → (Fin 1 ⊕ Fin 3))
    ...
    h.derivSubmodule n
      ≤ LinearMap.ker (LinearMap.mulLeft ℂ (h.covH k d φ)
        - LinearMap.mulRight ℂ (h.covH k d φ)) := by
  rw [derivSubmodule]
  refine sup_le ?_ ?_
  · rw [higgsSubmodule]
...
lemma derivSubmodule_le_ker_barH {n k : ℕ} (d : Fin k → (Fin 1 ⊕ Fin 3))
    ...
    h.derivSubmodule n
      ≤ LinearMap.ker (LinearMap.mulLeft ℂ (h.covBarH k d φ)
        - LinearMap.mulRight ℂ (h.covBarH k d φ)) := by
  rw [derivSubmodule]
  refine sup_le ?_ ?_
  · rw [higgsSubmodule]
    refine iSup_le fun d' => ?_
