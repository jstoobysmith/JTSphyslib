...
The fermionic species are indexed by `FermionType × Fin 3`: fifteen multiplets, one of the
five types in one of the three generations, each occurring exactly once. The matter field
does not depend on the generation, three generations being three copies of one multiplet
distinguished only by their Yukawa couplings, which are not field data. The bosonic family
has the single Higgs multiplet. The gauge bosons are not a species: their generator space
is fixed by the gauge algebra alone, and `GaugeFieldData` supplies it as the connection
sector.
...
- `StandardModel.FermionType` : the five fermion types.
...
/-- The five fermion types of the Standard Model. Each occurs in three generations, which
  are indexed separately. -/
inductive FermionType where
  /-- The lepton doublet, `(1, 2)_{-3}`. -/
  | leptonDoublet : FermionType
  /-- The charged-lepton singlet, `(1, 1)_{-6}`. -/
  | leptonSinglet : FermionType
  /-- The quark doublet, `(3, 2)_{1}`. -/
  | quarkDoublet : FermionType
  /-- The up-type quark singlet, `(3, 1)_{4}`. -/
  | upSinglet : FermionType
  /-- The down-type quark singlet, `(3, 1)_{-2}`. -/
  | downSinglet : FermionType
deriving DecidableEq

namespace FermionType

/-- There are five fermion types. -/
instance : Fintype FermionType where
  elems := {.leptonDoublet, .leptonSinglet, .quarkDoublet, .upSinglet, .downSinglet}
  complete := fun t => by cases t <;> decide

/-- The matter field of a fermion type, one of the five existing adapters. It is the same
  in every generation. -/
noncomputable def matterField : FermionType → MatterField JetGaugeGroupI
  | .leptonDoublet => LeptonDoublet.matterField
  | .leptonSinglet => LeptonSinglet.matterField
  | .quarkDoublet => QuarkDoublet.matterField
  | .upSinglet => UpSinglet.matterField
  | .downSinglet => DownSinglet.matterField

@[simp]
lemma matterField_leptonDoublet :
    matterField .leptonDoublet = LeptonDoublet.matterField := rfl

@[simp]
lemma matterField_leptonSinglet :
    matterField .leptonSinglet = LeptonSinglet.matterField := rfl

@[simp]
lemma matterField_quarkDoublet :
    matterField .quarkDoublet = QuarkDoublet.matterField := rfl

@[simp]
lemma matterField_upSinglet : matterField .upSinglet = UpSinglet.matterField := rfl

@[simp]
lemma matterField_downSinglet : matterField .downSinglet = DownSinglet.matterField := rfl

/-- Every Standard Model fermion carries mass weight three. -/
@[simp]
...
noncomputable def fieldData : GaugeFieldData gaugeJet where
  FermionSpecies := FermionType × Fin 3
  fermion := fun t => t.1.matterField
  BosonSpecies := Unit
  boson := fun _ => HiggsVec.matterField

@[simp]
lemma fieldData_fermionSpecies : fieldData.FermionSpecies = (FermionType × Fin 3) := rfl

/-- A fermionic species is the multiplet of its type, in every generation. -/
@[simp]
lemma fieldData_fermion (t : FermionType) (i : Fin 3) :
    fieldData.fermion (t, i) = t.matterField := rfl
...
lemma card_fieldData_fermionSpecies : Nat.card fieldData.FermionSpecies = 15 := by
  show Nat.card (FermionType × Fin 3) = 15
...
lemma fieldData_fermion_massWeight (j : fieldData.FermionSpecies) :
  ...
  FermionType.matterField_massWeight j.1
...
/-- A fermionic component function `∂_s ψ_α` scales by `c ^ (3 + 2 |s|)`, whichever type
