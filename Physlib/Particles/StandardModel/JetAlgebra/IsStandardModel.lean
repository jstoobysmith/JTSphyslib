...
The abstract theory of `IsStandardModel` asks an algebra for an equivariant algebra map out
of the jet algebra of the Standard Model. The jet algebra therefore carries one for free —
the identity — and `JetAlgebra.isStandardModel` records it. The four compatibility laws are
...
The abstract theory `IsStandardModel` is written in terms of an equivariant algebra map out
...
/-- The jet algebra of the Standard Model is a Standard Model: it is one along the identity
  algebra map, since `IsStandardModel` asks precisely for an equivariant algebra map out of
  the jet algebra.
...
noncomputable def isStandardModel : IsStandardModel JetAlgebra repJetGaugeGroupI repLorentzGroup
...
with no mention of an `IsStandardModel` instance, and section B.3 gives them: a reader of
