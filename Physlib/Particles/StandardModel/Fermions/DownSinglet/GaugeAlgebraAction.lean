...
private lemma jetValLinEquiv_jetDeriv (μ : Fin 1 ⊕ Fin 3)
  ...
  | tmul f d =>
    ...
    | tmul ψ c =>
      rw [show jetDeriv μ (f ⊗ₜ[ℂ] (⟨ψ ⊗ₜ[ℂ] c⟩ : DownSinglet))
          = (pderiv μ f) ⊗ₜ[ℂ] (⟨ψ ⊗ₜ[ℂ] c⟩ : DownSinglet) from rfl,
        show jetValLinEquiv ((pderiv μ f) ⊗ₜ[ℂ] (⟨ψ ⊗ₜ[ℂ] c⟩ : DownSinglet))
...
lemma downMatrix_map_pderiv (U : JetGaugeGroupI) (μ : Fin 1 ⊕ Fin 3) :
    (downMatrix U).map (fun f => pderiv μ f)
  ...
  have hmap : ((((star ((U.2.2 : unitary JetRing) : JetRing)) ^ 2) •
      ...
      = (pderiv μ ((star ((U.2.2 : unitary JetRing) : JetRing)) ^ 2)) • U.1.1
        + ((star ((U.2.2 : unitary JetRing) : JetRing)) ^ 2)
  ...
  linear_combination (-(2 * pderiv μ ((U.2.2 : unitary JetRing) : JetRing)
    * star ((U.2.2 : unitary JetRing) : JetRing)
    * star ((U.2.2 : unitary JetRing) : JetRing)
  ...
  | tmul ψ c =>
    ...
    rw [show ((Matrix.toLpLinAlgEquiv 2 (downMatrix U))
      ...
      show ((Matrix.toLpLinAlgEquiv 2 ((downMatrix U).map fun f =>
          constantCoeff (x.foldl (fun h ρ => pderiv ρ h) f))) c).ofLp j
        = ∑ k, constantCoeff (x.foldl (fun h ρ => pderiv ρ h) (downMatrix U j k))
            * c.ofLp k from by
        simp [Matrix.mulVec_eq_sum, Finset.sum_apply, mul_comm],
  ...
  · intro U μ x
    ...
    have hMcons : ((downMatrix U).map fun f =>
      ...
      rw [show ((downMatrix U).map fun f =>
            constantCoeff ((μ ::ₘ x).foldl (fun h ρ => pderiv ρ h) f))
          = (((downMatrix U).map fun f => pderiv μ f).map fun f =>
    ...
    have hMact : ((downMatrix U).map fun f =>
      ...
      have h1 : ((downMatrix U * jetActionMatrix (JetGaugeAlgebra.ofConstant c)).map
        ...
        rw [hconst, JetRing.matrix_constantCoeff_foldl_pderiv_mul,
          ...
          ite_eq_left rfl]
      rw [← h1, downMatrix_mul_jetActionMatrix, JetRing.matrix_constantCoeff_foldl_pderiv_mul]
...
## The gauge action commutes with the Lorentz action

-/

/-- The infinitesimal gauge action on the down-type singlet acts on the colour factor, the
