...
The jet ring is the ring generated by `x^μ`.
We can consider an element of `χ : JetRing`
as a formal power series in the spacetime coordinates.

The action `derivAction` below takes `∂_μ · ↦ ∂_μ (χ • ·)`.
...
/--
  For `χ : JetRing`, the action which takes `∂_μ · ↦ ∂_μ (χ • ·)`.
  We expand `∂_μ (χ • ·)` out using the chain rule. -/
