...
the Weyl reflection of `SU(2)`. On a doublet it sends `(a, b)` to `(-b, a)`: it exchanges
the two isospin components, and so exchanges the isospin weights `+1` and `-1` that
`IsospinDecomposition` records.
...
Like the isospin grading, this is a *sieve* rather than a characterization:
`mem_zero_of_invariant` says an invariant element has grade zero, and there is no converse.
What it buys is a genuine sharpening of `IsospinDecomposition`, whose weight-zero piece
cannot separate the isospin singlet from the neutral component of a higher multiplet:
`su2Perm` exchanges the two isospin components, so it acts on the isospin-zero subspace and
its grade-`2` part is thrown away by this sieve.
