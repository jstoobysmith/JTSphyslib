...
- `IsStandardModel.covDerivH`, `IsStandardModel.covDerivFieldStrength` and their
  companions : the covariant derivative towers.
- `IsStandardModel.fieldAlgebra_eq_covDeriv` : the covariant towers generate the field
  algebra.
- `IsStandardModel.invariant_mem_adjoin_covDeriv` : the classification of jet-gauge
  invariants of the field algebra.
- `IsStandardModel.repLorentz_covDerivH` and its companions : the Lorentz laws of the
