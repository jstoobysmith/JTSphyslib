...
exchange: the two sets of generators generate the same algebra, and a jet-gauge invariant of
it is a polynomial in the covariant towers.
...
written as an operator `lorentzMix` on multiset-indexed families. It is a morphism for the
Leibniz convolution out of which the correction terms of a covariant derivative are built.
The Lorentz law of a covariant tower is then a single induction, `repLorentz_tower`, run
once for an abstract tower and instantiated twice: for the matter towers, where the value
index carries the contragredient action and the gauge action has to commute with the Lorentz
action, and for the field-strength tower, where the adjoint index carries no Lorentz weight.

Sections G onwards work inside a Standard Model. The field algebra is the algebra generated
...
in both, does not change the algebra generated. The towers are gauge covariant, transform
through the base point of a gauge jet alone, and are fixed by a pure gauge jet — the three
facts `AlgebraRealization.CovFieldAlgebra.Basic` combines into the classification of
jet-gauge invariants. The last two sections record the Lorentz laws of the covariant matter
...
- G. What a covariant tower inherits from its family
- H. The field algebra and the covariant towers
- I. The covariant towers generate the field algebra
- J. Gauge covariance of the covariant towers
- K. The gauge action commutes with the Lorentz action
- L. The Lorentz laws of the covariant matter towers
...
sum over tuples, but the covariant derivative symbols carry multisets of directions, where
no ordering is available. The mixing is therefore an operator on multiset-indexed families:
peel one direction `a`, replace it by every direction `b` weighted by `L[Λ] b a`, and mix
what is left. Peeling two directions commutes, so the recursion descends to multisets;
`lorentzMix_ofFn` identifies the operator with the tuple form, and the remaining lemmas
record how it interacts with translation of the index and that it is linear in the family.
...
lemma lorentzMix_cons_apply (a : Fin 1 ⊕ Fin 3) (s t : Multiset (Fin 1 ⊕ Fin 3)) :
    lorentzMix Λ G (a ::ₘ s) t = ∑ b, L[Λ] b a • lorentzMix Λ G s (b ::ₘ t) := by
...
lemma lorentzMix_apply_add (s : Multiset (Fin 1 ⊕ Fin 3)) :
  ...
  | cons a s ih =>
      ...
      rw [ih G, ih (fun r => G (r + t))]
      simp only [Multiset.add_cons, Multiset.cons_add, add_zero]
...
lemma lorentzMix_cons_zero (a : Fin 1 ⊕ Fin 3) (s : Multiset (Fin 1 ⊕ Fin 3)) :
    lorentzMix Λ G (a ::ₘ s) 0 =
      ∑ b, L[Λ] b a • lorentzMix Λ (fun t => G (b ::ₘ t)) s 0 := by
  rw [lorentzMix_cons_apply]
  refine Finset.sum_congr rfl fun b _ => ?_
  rw [lorentzMix_apply_add Λ s G (b ::ₘ 0)]
  simp only [Multiset.add_cons, add_zero]
...
lemma lorentzMix_sub_fam (G₁ G₂ : Multiset (Fin 1 ⊕ Fin 3) → M)
    (s t : Multiset (Fin 1 ⊕ Fin 3)) :
    lorentzMix Λ (fun r => G₁ r - G₂ r) s t =
      lorentzMix Λ G₁ s t - lorentzMix Λ G₂ s t := by
...
carrying `y`, summed over all splittings `s = x + y`. Expanded in bases of the gauge
algebra and of the value space, both correction terms of this file are scalar combinations
of such convolutions of plain products in `B`, which is why `lorentzMix_derivConv` — the
mixing operator is a morphism for the convolution — is what carries a Lorentz law through
...
lemma derivConv_sum_right {ι : Type*} [Fintype ι] (f : Multiset (Fin 1 ⊕ Fin 3) → B)
    (c : ι → ℂ) (g : ι → Multiset (Fin 1 ⊕ Fin 3) → B)
    (s : Multiset (Fin 1 ⊕ Fin 3)) :
...
lemma derivConv_sum_left {ι : Type*} [Fintype ι] (g : Multiset (Fin 1 ⊕ Fin 3) → B)
    (c : ι → ℂ) (f : ι → Multiset (Fin 1 ⊕ Fin 3) → B)
    (s : Multiset (Fin 1 ⊕ Fin 3)) :
...
tower along `l'`. Both towers of this file have that shape — the matter towers with the
derived action of the gauge field as correction, the field-strength tower with the derived
bracket — and both corrections are Lorentz covariant, linear in the family they correct,
and compatible with a twist of the value index. That is all the induction uses, so it is
run once, for an abstract tower `T` transforming into a possibly different tower `T'`: the
ordered covariant slots mix by their own columns of the Lorentz matrix, the multiset of
plain derivative slots by `lorentzMix`, and the value index by the twist `τ`.
...
theorem repLorentz_tower {K : Type} [Field K] {W : Type} [AddCommGroup W] [Module K W]
    ...
    (T T' : (n : ℕ) → (Fin n → (Fin 1 ⊕ Fin 3)) → Multiset (Fin 1 ⊕ Fin 3) →
      W →ₗ[K] B)
    ...
    (hstep' : ∀ (n : ℕ) (l : Fin (n + 1) → (Fin 1 ⊕ Fin 3))
      (s : Multiset (Fin 1 ⊕ Fin 3)), T' (n + 1) l s =
        T' n (fun i => l i.succ) (l 0 ::ₘ s) + C (l 0) (T' n fun i => l i.succ) s)
...
infinitesimal action `act`. Expanded in bases of the gauge algebra and of the value space,
the correction is a scalar combination of Leibniz convolutions of gauge-field symbols
against matter symbols, which gives its Lorentz law and its linearity in the matter family.
The twist of the value index is the contragredient action `rep.dual Λ`, and it passes
through the correction because the gauge action commutes with the Lorentz action on the
value space: that is the one hypothesis about the species that the Lorentz law of its
covariant tower needs.
...
lemma repLorentz_sum_derivConv
  ...
  have h1 : ∀ j k, repLorentz Λ (derivConv (fun x => A x ρ (bg.coord j)) (g k) s) =
    ...
    rw [show derivConv (fun t => ∑ a, L[Λ] a ρ • A t a (bg.coord j)) (g' k) =
        fun r => ∑ a, L[Λ] a ρ • derivConv (fun x => A x a (bg.coord j)) (g' k) r from
      funext fun r => derivConv_sum_left _ _ _ _, lorentzMix_sum_fam]
    exact Finset.sum_congr rfl fun a _ => lorentzMix_smul_fam _ _ _ _ _
...
lemma dual_twist {κ : Type} [Fintype κ] (bv : Module.Basis κ ℂ V) (T : V →ₗ[ℂ] V)
  ...
  rw [show ∑ k, φ (act c (bv k)) • T.dualMap (bv.coord k) =
        T.dualMap (∑ k, φ (act c (bv k)) • bv.coord k) by simp only [map_sum, map_smul],
    show (∑ k, φ (act c (bv k)) • bv.coord k) = φ ∘ₗ act c from
...
lemma actionFam_comp_dual (T : V →ₗ[ℂ] V)
  ...
  set bg := Module.finBasis ℝ GaugeAlgebra
  set bv := Module.finBasis ℂ V
  have key : ∀ (α : Fin (Module.finrank ℂ V) → ℂ) (v : Fin (Module.finrank ℂ V) → Module.Dual ℂ V)
      (j : Fin (Module.finrank ℝ GaugeAlgebra)),
      ∑ k, α k • (f (bg.coord j) * g (v k)) = f (bg.coord j) * g (∑ k, α k • v k) := by
    intro α v j
    simp only [map_sum, map_smul, Finset.mul_sum, mul_smul_comm]
  simp only [actionFam_apply_eq_sum bg bv, LinearMap.comp_apply, key, dual_twist bv T hT]
...
The covariant derivative of an adjoint family is the same shape as that of a matter family,
with the action of the gauge field on the value index replaced by the bracket `⁅A_ρ, ·⁆`;
the gauge index carries no Lorentz weight, so the twist is the identity, and the induction
of section C applies with `bracketFamConv` in place of `actionFamConv`. What is new is the
seed: the field strength itself carries two covector indices, and its Lorentz law
(`repLorentz_fieldStrength_mix`) mixes both, the derivative terms through
`repLorentz_apply_mix` and the commutator term through the bracket convolution.
...
lemma repLorentz_fieldStrength_mix
  ...
  have hder : ∀ κ σ, repLorentz Λ (A (κ ::ₘ x) σ φ) =
    ...
    intro κ σ
    rw [repLorentz_apply_mix hA Λ (κ ::ₘ x) σ φ, lorentzMix_cons_zero, lorentzMix_sum_fam]
    exact Finset.sum_congr rfl fun a _ => (lorentzMix_smul_fam _ _ _ _ _).symm
  -- the commutator term
  have hcomm : repLorentz Λ (commutatorFam A μ ν x φ) =
    ...
    rw [show commutatorFam A μ ν x = bracketFamConv A μ (fun r => A r ν) x from rfl,
      repLorentz_bracketFamConv hmul hA Λ μ _ _ hG x φ, lorentzMix_sum_fam]
    refine Finset.sum_congr rfl fun a _ => ?_
    rw [lorentzMix_smul_fam]
    exact congrArg (fun G => L[Λ] a μ • lorentzMix Λ G x 0)
      (funext fun t => bracketFamConv_sum_fam (A := A) a _ (fun b r => A r b) t φ)
...
lemma repLorentz_iteratedCovDerivAdjoint_fieldStrength
  ...
  have hF' : ∀ y χ, repLorentz Λ (fieldStrength A μ ν y χ) =
    ...
    fun y χ => by
      simpa only [LinearMap.sum_apply, LinearMap.smul_apply] using
...
The field strength is antisymmetric in its two covector indices as soon as the symbols of
the gauge field commute with one another in `B`: the two derivative terms swap outright,
and the commutator term swaps by the antisymmetry of the gauge-algebra bracket, once the
two factors of each product may be exchanged. The covariant tower inherits the
antisymmetry, the iterated covariant derivative being linear in the family it
differentiates.

-/

/-- The bracket of two component families with commuting values is antisymmetric: in the
  basis expansion the structure constants are antisymmetric in the two gauge indices, and
  the two field factors of each term may be exchanged. -/
lemma bracketFam_swap_of_commute {f g : Module.Dual ℝ GaugeAlgebra →ₗ[ℝ] B}
  ...
  rw [(hfg _ _).eq, ← lie_skew, map_neg, neg_smul]
...
/-- The field strength is antisymmetric in its two covector indices when the symbols of
  the gauge field commute: the two derivative terms swap outright, the commutator term by
  `commutatorFam_swap`. -/
...
lemma iteratedCovDerivAdjoint_neg_fam
  ...
  have h1 : (fun t => - F t) = fun t => ∑ _i : Fin 1, (-1 : ℂ) • F t := by
    funext t; simp
  rw [h1, iteratedCovDerivAdjoint_sum_fam (A := A) (fun _ : Fin 1 => (-1 : ℂ)) (fun _ => F) l x φ]
  simp
...
Three facts about the covariant tower of a single matter family, in the form the field
algebra consumes. The span lemma `IsGaugeField.adjoin_symbols_eq_adjoin_covDerivIter` says
that the bare symbols and the tower generate the same algebra over the gauge-field symbols,
so each is a polynomial in the other: `symbol_mem_adjoin` and `covDerivIter_mem_adjoin`.
The tower commutes with the gauge-field symbols as soon as the bare symbols do, the tower
being a polynomial in symbols that commute with them. And a gauge tensor transforms at the
base point through the dual coefficient of the base-point value alone, the Leibniz
convolution over the empty multiset having a single term.
...
/-- A matter gauge tensor transforms at the base point through the dual coefficient of the
  base-point value of the gauge jet alone. -/
lemma _root_.StandardModel.TransformsIn.repGauge_zero
    {rep : Representation ℂ JetGaugeGroupI (JetRing ⊗[ℂ] V)}
    {F : Multiset (Fin 1 ⊕ Fin 3) → Module.Dual ℂ V →ₗ[ℂ] B} (hF : TransformsIn repGauge rep F)
...
infinitesimal action of its species (`GaugeAlgebra.actionConj` of it for a conjugate
family), together with the iterated covariant derivative of the field strength along a list
of directions. The set `matterTowers` collects the twelve matter towers, and
`matterTowers_induction` is the case split over them that the rest of the file runs.
...
noncomputable def covDerivBarH {n : ℕ} (l : Fin n → (Fin 1 ⊕ Fin 3)) :
  ...
  IsGaugeField.covDerivIter h.A (GaugeAlgebra.actionConj HiggsVec.gaugeAlgebraAction) h.barH n l 0
...
For a single matter family the span lemma
`IsGaugeField.adjoin_symbols_eq_adjoin_covDerivIter` says that its bare symbols and its
covariant tower generate the same algebra together with the gauge-field symbols. The field
algebra is that statement for the twelve families at once: each bare symbol is a polynomial
in the gauge-field symbols and its own tower, and each tower symbol is a polynomial in the
gauge-field symbols and its own bare symbols.
...
lemma fieldAlgebra_eq_covDeriv :
  ...
  rw [Set.union_assoc]
  show Algebra.adjoin ℂ h.symbols = Algebra.adjoin ℂ
    ((⋃ (s : Multiset (Fin 1 ⊕ Fin 3)) (μ : Fin 1 ⊕ Fin 3), Set.range (h.A s μ)) ∪
      h.matterTowers)
  ...
  -- every bare symbol is a polynomial in the gauge-field symbols and its tower
  · rintro b ((hb | hb) | hb)
    · exact Algebra.subset_adjoin (Or.inl hb)
    · simp only [Set.mem_iUnion] at hb
      obtain ⟨s, ⟨φ, rfl⟩ | ⟨φ, rfl⟩⟩ := hb
      · exact IsGaugeField.symbol_mem_adjoin HiggsVec.gaugeAlgebraAction h.H hA
          (fun n l φ => Or.inr (Or.inl (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivH]))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin (GaugeAlgebra.actionConj HiggsVec.gaugeAlgebraAction)
          h.barH hA (fun n l φ => Or.inr (Or.inl (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivBarH]))))) s φ
    · simp only [Set.mem_iUnion] at hb
      obtain ⟨i, s, hb⟩ := hb
      rcases hb with (((((((((⟨φ, rfl⟩ | ⟨φ, rfl⟩) | ⟨φ, rfl⟩) | ⟨φ, rfl⟩) | ⟨φ, rfl⟩) |
        ⟨φ, rfl⟩) | ⟨φ, rfl⟩) | ⟨φ, rfl⟩) | ⟨φ, rfl⟩) | ⟨φ, rfl⟩)
      · exact IsGaugeField.symbol_mem_adjoin DownSinglet.gaugeAlgebraAction (h.d i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivD])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin
          (GaugeAlgebra.actionConj DownSinglet.gaugeAlgebraAction) (h.bard i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivBarD])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin UpSinglet.gaugeAlgebraAction (h.u i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivU])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin
          (GaugeAlgebra.actionConj UpSinglet.gaugeAlgebraAction) (h.baru i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivBarU])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin QuarkDoublet.gaugeAlgebraAction (h.Q i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivQ])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin
          (GaugeAlgebra.actionConj QuarkDoublet.gaugeAlgebraAction) (h.barQ i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivBarQ])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin LeptonDoublet.gaugeAlgebraAction (h.L i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivL])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin
          (GaugeAlgebra.actionConj LeptonDoublet.gaugeAlgebraAction) (h.barL i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivBarL])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin LeptonSinglet.gaugeAlgebraAction (h.e i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivE])))))) s φ
      · exact IsGaugeField.symbol_mem_adjoin
          (GaugeAlgebra.actionConj LeptonSinglet.gaugeAlgebraAction) (h.bare i) hA
          (fun n l φ => Or.inr (Or.inr (Set.mem_iUnion_of_mem i (Set.mem_iUnion_of_mem n
            (Set.mem_iUnion_of_mem l (by simp [covDerivBarE])))))) s φ
  ...
  · rintro b (hb | hb)
    ...
    · refine h.matterTowers_induction (fun b => b ∈ Algebra.adjoin ℂ h.symbols) hb
        ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_
      · exact fun n l φ => IsGaugeField.covDerivIter_mem_adjoin _ _ hA' (fun s φ =>
          Or.inl (Or.inr (Set.mem_iUnion_of_mem s (by simp)))) n l φ
      · exact fun n l φ => IsGaugeField.covDerivIter_mem_adjoin _ _ hA' (fun s φ =>
          Or.inl (Or.inr (Set.mem_iUnion_of_mem s (by simp)))) n l φ
      all_goals exact fun i n l φ => IsGaugeField.covDerivIter_mem_adjoin _ _ hA' (fun s φ =>
...
lemma repJet_covDerivH {n : ℕ} (l : Fin n → (Fin 1 ⊕ Fin 3)) (U : JetGaugeGroupI)
    ...
    repJet U (h.covDerivH l φ) =
      h.covDerivH l (IsGaugeField.repDualCoeff HiggsVec.repJetGaugeGroupI U⁻¹ 0 φ) :=
  (h.transformsIn_covDerivH n l).repGauge_zero U φ

/-- The covariant tower of the conjugate Higgs transforms through the base point of a gauge
  jet. -/
...
lemma repJet_covDerivD (i : Fin 3) {n : ℕ} (l : Fin n → (Fin 1 ⊕ Fin 3)) (U : JetGaugeGroupI)
    ...
    repJet U (h.covDerivD i l φ) =
      h.covDerivD i l (IsGaugeField.repDualCoeff DownSinglet.repJetGaugeGroupI U⁻¹ 0 φ) :=
  (h.transformsIn_covDerivD i n l).repGauge_zero U φ

/-- The covariant tower of the conjugate down-type quarks transforms through the base point
  of a gauge jet. -/
...
lemma repJet_covDerivU (i : Fin 3) {n : ℕ} (l : Fin n → (Fin 1 ⊕ Fin 3)) (U : JetGaugeGroupI)
    ...
    repJet U (h.covDerivU i l φ) =
      h.covDerivU i l (IsGaugeField.repDualCoeff UpSinglet.repJetGaugeGroupI U⁻¹ 0 φ) :=
  (h.transformsIn_covDerivU i n l).repGauge_zero U φ

/-- The covariant tower of the conjugate up-type quarks transforms through the base point
  of a gauge jet. -/
...
lemma repJet_covDerivQ (i : Fin 3) {n : ℕ} (l : Fin n → (Fin 1 ⊕ Fin 3)) (U : JetGaugeGroupI)
    ...
    repJet U (h.covDerivQ i l φ) =
      h.covDerivQ i l (IsGaugeField.repDualCoeff QuarkDoublet.repJetGaugeGroupI U⁻¹ 0 φ) :=
  (h.transformsIn_covDerivQ i n l).repGauge_zero U φ

/-- The covariant tower of the conjugate quark doublets transforms through the base point
  of a gauge jet. -/
...
lemma repJet_covDerivL (i : Fin 3) {n : ℕ} (l : Fin n → (Fin 1 ⊕ Fin 3)) (U : JetGaugeGroupI)
    ...
    repJet U (h.covDerivL i l φ) =
      h.covDerivL i l (IsGaugeField.repDualCoeff LeptonDoublet.repJetGaugeGroupI U⁻¹ 0 φ) :=
  (h.transformsIn_covDerivL i n l).repGauge_zero U φ

/-- The covariant tower of the conjugate lepton doublets transforms through the base point
  of a gauge jet. -/
...
lemma repJet_covDerivE (i : Fin 3) {n : ℕ} (l : Fin n → (Fin 1 ⊕ Fin 3)) (U : JetGaugeGroupI)
    ...
    repJet U (h.covDerivE i l φ) =
      h.covDerivE i l (IsGaugeField.repDualCoeff LeptonSinglet.repJetGaugeGroupI U⁻¹ 0 φ) :=
  (h.transformsIn_covDerivE i n l).repGauge_zero U φ

/-- The covariant tower of the conjugate lepton singlets transforms through the base point
  of a gauge jet. -/
...
lemma repJet_covDerivBarH_of_mem_truncationKer_zero {n : ℕ} (l : Fin n → (Fin 1 ⊕ Fin 3))
  ...
  (h.transformsIn_covDerivBarH n l).repGauge_eq_of_mem_truncationKer_zero
    (fun hW => GaugeAlgebra.repCoeff_repConj_zero_eq_id
...
lemma covDerivFieldStrength_swap (l : List (Fin 1 ⊕ Fin 3)) (μ ν : Fin 1 ⊕ Fin 3)
  ...
  rw [covDerivFieldStrength, covDerivFieldStrength,
    show IsGaugeField.fieldStrength h.A ν μ = fun t => - IsGaugeField.fieldStrength h.A μ ν t from
...
The Lorentz law of a covariant matter tower needs the infinitesimal gauge action of its
species to commute with the Lorentz action on the value space. Each species proves this
commutation for itself, next to its `gaugeAlgebraAction`
...
Conjugation transports the commutation to the conjugate families.
...
Each covariant matter tower is an iterated covariant derivative of the corresponding bare
family, so `IsGaugeField.isLorentzCovDerivTransforms_covDerivIter` turns the bare Lorentz
law recorded by `AlgebraRealization` into the covariant one, given the commutation of the
gauge action with the Lorentz action of section K; for the conjugate towers that
commutation is transported by `actionConj_comm_repConj`.
...
/-- The covariant tower of the conjugate down-type quarks transforms in the conjugate Weyl
  representation. -/
