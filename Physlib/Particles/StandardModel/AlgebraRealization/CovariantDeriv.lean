...
through the base point of a gauge jet alone, and are fixed by a pure gauge jet. Those combine
into `invariant_mem_adjoin_covDeriv`: a jet-gauge invariant of the field algebra lies in the
algebra generated by the covariant towers, gauge invariance having eliminated the bare
gauge-field symbols. The last two sections record the Lorentz laws of the covariant matter
...
- K. The classification of gauge invariants
- L. The gauge action commutes with the Lorentz action
- M. The Lorentz laws of the covariant matter towers
...
The matter towers commute with the gauge-field symbols and are fixed by pure gauge jets, so
the abstract classification `IsGaugeField.invariant_mem_adjoin_fieldStrength` applies to
the field algebra written in terms of the covariant towers: a jet-gauge invariant is a
polynomial in the covariant towers of the field strength and of the matter fields, gauge
invariance having eliminated the bare gauge-field symbols.

-/

/-- The classification of gauge invariants of the field algebra: a `repJet`-invariant
  element of the field algebra is a polynomial in the covariant derivatives of the field
  strength and the covariant derivatives of the matter fields. -/
...
species to commute with the Lorentz action on the value space. The two act on different
tensor factors: the Lorentz group on the Weyl factor and the gauge algebra on the colour and
weak factors, so they commute; for the Higgs the Lorentz action is trivial, and for the
charged-lepton singlet the gauge action is a scalar. Conjugation transports the commutation
to the conjugate families.
...
/-- Reassociating and recombining the last two tensor factors commutes with an
  endomorphism of the first: the shape needed for the quark doublet, whose gauge action is
  read on the combined colour–weak factor. -/
...
gauge action with the Lorentz action of section L; for the conjugate towers that
