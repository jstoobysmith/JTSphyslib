...
def lift {A : Type} [Ring A] [Algebra ℂ A] (F : FieldGenerators → A)
    (hFB : ∀ g g', Commute (F (.cScalar g)) (F (.cScalar g')))
  ...
  haveI : IsMulCommutative (Algebra.adjoin ℂ (Set.range FB)) :=
    Algebra.isMulCommutative_adjoin ℂ (by rintro _ ⟨g, rfl⟩ _ ⟨g', rfl⟩; exact hFB g g')
  let fS := SymmetricAlgebra.lift (complexScalarComponentBasis.constr ℂ fun g =>
    (⟨FB g, Algebra.subset_adjoin (Set.mem_range_self g)⟩ : Algebra.adjoin ℂ (Set.range FB)))
  Algebra.TensorProduct.lift
    ...
    (ExteriorAlgebra.lift ℂ
      ⟨fermionicComponentBasis.constr ℂ FF, fun v => by
        ...
        rw [Basis.constr_apply_fintype, Finset.sum_mul_sum, ← Finset.sum_product']
