...
theorem adiabatic_relation_log
  ...
  -- Unfold `entropy`, then cancel the common summand `N * s0` and factor `N * R`
  -- to isolate the bracketed expression, on which the two states agree.
  unfold entropy at hS
  have hNR : N * R ≠ 0 := mul_ne_zero hN.ne' hR.ne'
  have key := mul_left_cancel₀ hNR (add_left_cancel hS)
  -- Expand every `log (·/·)` into a difference of logs; the goal then differs
  -- from `key` only by the common `(c + 1) * log (N / N0)` terms, which cancel.
  rw [Real.log_div hUa.ne' hUb.ne', Real.log_div hVa.ne' hVb.ne']
  rw [Real.log_div hUa.ne' hU0.ne', Real.log_div hUb.ne' hU0.ne',
      Real.log_div hVa.ne' hV0.ne', Real.log_div hVb.ne' hV0.ne'] at key
