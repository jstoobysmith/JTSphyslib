...
`f : V →ₗ W` intertwines two fibrewise actions on the jets, then the pullback of component
...
symbols. The conjugate half is the unconjugated argument applied to `repConj repV` and
`repConj repW`, whose intertwining is `JetComponentSpace.lTensor_comp_repConj`. -/
lemma comap_comp_repJet [Module.Free ℂ V] [Module.Finite ℂ V]
    [Module.Free ℂ W] [Module.Finite ℂ W]
    (repV : Representation ℂ G (JetRing ⊗[ℂ] V))
    (hV : ∀ (U : G) (χ : JetRing) (z : JetRing ⊗[ℂ] V), repV U (χ • z) = χ • repV U z)
    (repW : Representation ℂ G (JetRing ⊗[ℂ] W))
    ...
    (hf : ∀ U : G, (LinearMap.lTensor JetRing f).comp (repV U)
      = (repW U).comp (LinearMap.lTensor JetRing f)) (U : G) :
    (comap f).comp (repJet repW hW U) = (repJet repV hV U).comp (comap f) := by
  show (LinearMap.prodMap (TensorProduct.map LinearMap.id (Module.Dual.transpose f))
      ...
      (LinearMap.prodMap (repDual repW hW U)
        (repDual (repConj repW) (repConj_smul_comm hW) U))
    = (LinearMap.prodMap (repDual repV hV U)
        (repDual (repConj repV) (repConj_smul_comm hV) U)).comp
  ...
  rw [LinearMap.prodMap_comp, LinearMap.prodMap_comp,
    comap_comp_repDual repV hV repW hW f hf U,
    comap_comp_repDual (repConj repV) (repConj_smul_comm hV) (repConj repW)
      (repConj_smul_comm hW) (ConjModule.map (k := ℂ) f)
      (lTensor_comp_repConj repV repW f hf) U]
