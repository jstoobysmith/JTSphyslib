...
completeness, leaving the span unchanged (C); `RankTwo` then supplies the
...
`RankTwo` classifies the invariants of `vectorPair`, and its metric trace is the Pauli
...
lemma repLorentz_pauliContraction (g : SL(2,ℂ)) :
  ...
  have hV := hT.isLorentzCovariant_vectorPair
  ...
  rw [← metricContraction_vectorPair (T := T), RankTwo.metricContraction, map_sum]
...
theorem exists_smul_pauliContraction_of_invariant {x : B} (hx : x ∈ hT.span)
  ...
  have hx' : x ∈ componentSpan (vectorPair (T := T)) := by
    ...
    exact hx
  obtain ⟨a, ha⟩ := RankTwo.exists_smul_metricContraction_of_invariant hV hx' hinv
...
lemma exists_smul_pauliContraction_of_invariant_subset {x : B} (S : Submodule ℂ B)
  ...
  have hx' : x ∈ componentSpan (vectorPair (T := T)) ⊔ S := by
    ...
    exact hx
  obtain ⟨a, y, hy, ha⟩ :=
    RankTwo.exists_smul_metricContraction_of_invariant_subset hV S hS hx' hinv
