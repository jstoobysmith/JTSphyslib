...
Results about matrices with entries in `JetRing`, chiefly the Euler (radial) transport: a
...
lemma sum_X_smul_mcMatrix_of_eulerTransport {κ : Type} [Fintype κ] [DecidableEq κ]
    {P V : Matrix κ κ JetRing} (hVu : V * star V = 1)
    (hEV : ∑ μ, (X μ : JetRing) • V.map (pderiv μ) = ((-Complex.I) • P) * V) :
