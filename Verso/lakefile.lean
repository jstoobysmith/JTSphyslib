...
require Physlib from ".."

package «physlib-verso» where
  -- Share the parent workspace's package clones and build products
  -- (mathlib etc. are already built there).
...
/--
The `wiki` facet renders the Physlib modules listed in `Verso/literate.toml`
as a static literate-programming website.

This is verso's own `literateHtml` package facet (see the verso package's
lakefile), adapted so that:
 * the modules come from the `Physlib` package (this repository's root
   package) rather than from this package, and
 * the configuration file and the generated site both live under `Verso/`.

Build it with `lake query :wiki` from inside `Verso/`; the path of the
generated site is printed on success.
-/
